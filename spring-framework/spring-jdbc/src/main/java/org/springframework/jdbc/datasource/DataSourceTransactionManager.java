...
public class DataSourceTransactionManager extends AbstractPlatformTransactionManager
	...
	protected Object doGetTransaction() {
		...
		// 用来保存当前事物的信息, 第一次从线程的变量中获取,由于第一次进来开始事物,事物的同步管理器没有被缓存,取出来的值是空的
	...
	protected void doBegin(Object transaction, TransactionDefinition definition) {
		...
		try {
			...
			if (!txObject.hasConnectionHolder() ||
				...
				// 把我们的数据库连接包装成一个setConnectionHolder对象,然后设置到txObject对象中
			...
			// 判断事物是否需要设置为只读事物 :SET TRANSACTION READ ONLY
			...
			// 绑定我们的数据源和连接到我们的同步管理器上,把数据源作为key,数据连接作为value 设置到线程变量中。
