...
public class ConfigurationClassPostProcessor implements BeanDefinitionRegistryPostProcessor,
	...
	public void postProcessBeanDefinitionRegistry(BeanDefinitionRegistry registry) {
        ...
        logger.info("refresh() 5.3 -->  根据对应的registry对象生成hashcode值，此对象只会操作一次，如果之前处理过则抛出异常");
		...
		logger.info("refresh() 5.3 --> 解析@Configuration, 标注的配置类,处理配置类的bean定义信息");
	...
	/**
	 * 从容器中获取所有的BeanDefinition 筛选符合规则的配置类添加到容器中
	 * 创建配置类解析器configurationClassParser ,创建集合用来存放配置类，和用来解析配置类
	 * parser.pares() 核心的解析流程 processConfigurationClass 开始各个注解的解析工作
	 *
	...
	public void processConfigBeanDefinitions(BeanDefinitionRegistry registry) {
		...
		logger.info("refresh() 5.4 --> 获取所有已经注册的BeanDefinition的BeanName");
		String[] candidateNames = registry.getBeanDefinitionNames();
		logger.info("refresh() 5.4 --> 构建和验证一个类是否被@Configuration 修饰,并做相关的解析工作");
		logger.info("refresh() 5.4 --> 判断当前beanDefinition 是否增加了@Configuration、@Bean、" +
        ...
        logger.info("refresh() 5.4 --> 如果没有发现任何配置类，则直接返回");
		if (configCandidates.isEmpty()) {
			return;
		}

		// Sort by previously determined @Order value, if applicable
		logger.info("refresh() 5.4 --> 如果同时增加了Order注解,进行排序");
		...
		if (registry instanceof SingletonBeanRegistry) {
            ...
            logger.info("refresh() 5.4 --> 判断是否有自定义的beanName生成器");
			if (!this.localBeanNameGeneratorSet) {
                logger.info("refresh() 5.4 --> 获取自定义的beanName生成器");
				BeanNameGenerator generator = (BeanNameGenerator) sbr.getSingleton(
						AnnotationConfigUtils.CONFIGURATION_BEAN_NAME_GENERATOR);
				if (generator != null) {
                    logger.info("refresh() 5.4 --> 设置组件扫描的beanName生成策略");
					this.componentScanBeanNameGenerator = generator;
                    logger.info("refresh() 5.4 --> 设置import bean name生成策略");
					this.importBeanNameGenerator = generator;
				}
			}
		}
        logger.info("refresh() 5.4 --> 如果环境对象等于空，那么就重新创建新的环境对象");
		if (this.environment == null) {
			this.environment = new StandardEnvironment();
		}
		logger.info("refresh() 5.4 --> 实例化ConfigurationClassParser类,初始化相关参数");
		...
		logger.info("refresh() 5.4 --> 循环解析,带有相关注解的类 @Configuration parse(candidates)-> ConfigurationClassParser");
