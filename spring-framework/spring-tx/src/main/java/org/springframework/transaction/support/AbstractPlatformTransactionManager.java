...
public abstract class AbstractPlatformTransactionManager implements PlatformTransactionManager, Serializable {
	...
	public final TransactionStatus getTransaction(@Nullable TransactionDefinition definition)
		...
		// 如果没有事物定义信息则使用默认的事物管理器定义信息
		TransactionDefinition def = (definition != null ? definition : TransactionDefinition.withDefaults());
		// 获取事物
		Object transaction = doGetTransaction();
		boolean debugEnabled = logger.isDebugEnabled();
		// 判断当前线程是否存在事物,判断依据是当前线程记录的连接不为空,切连接中的transactionActive属性不为空
		...
		// 如果当前线程不存在事物,但是事物的传播特性被设置为 PROPAGATION_MANDATORY (使用当前事物,如果不存在就抛出异常) 抛出异常
		if (def.getPropagationBehavior() == TransactionDefinition.PROPAGATION_MANDATORY) {
			throw new IllegalTransactionStateException(
					"No existing transaction found for transaction marked with propagation 'mandatory'");
		}
		// 如果不存在就创建一个,不管存不存在都新建一个,如果存在就使用之前的,没有创建新的
		else if (def.getPropagationBehavior() == TransactionDefinition.PROPAGATION_REQUIRED ||
				def.getPropagationBehavior() == TransactionDefinition.PROPAGATION_REQUIRES_NEW ||
				def.getPropagationBehavior() == TransactionDefinition.PROPAGATION_NESTED) {
			// 如果没有当前的事物 挂起的是空事物,创建新的
	...
	private TransactionStatus handleExistingTransaction(
			...
			throws TransactionException {
		// 判断当前事物隔离级别, PROPAGATION_NEVER,不支持事物,但是当前又存在一个事物抛出异常
		...
		if (definition.getPropagationBehavior() == TransactionDefinition.PROPAGATION_NOT_SUPPORTED) {
			...
			boolean newSynchronization = (getTransactionSynchronization() == SYNCHRONIZATION_ALWAYS);
			// 创建一个新的非事物状态(保存了上一个事物的属性)
			return prepareTransactionStatus(
					definition, null, false, newSynchronization, debugEnabled, suspendedResources);
		}
		// 当前事物状态 PROPAGATION_REQUIRES_NEW  需要开启一个新的事物
		...
		// 当前事物状态 PROPAGATION_NESTED 嵌套事务
	...
	public final void commit(TransactionStatus status) throws TransactionException {
		...
		// 如果在事物链中已经被标记回滚,那么不会尝试提交事物,直接回滚
	...
	private void processCommit(DefaultTransactionStatus status) throws TransactionException {
		try {
			...
			// 根据条件,完成后数据清楚,和线程私有资源解绑,重置连接自动提交,隔离级别,是否只读,释放连接,恢复挂起事物等。
