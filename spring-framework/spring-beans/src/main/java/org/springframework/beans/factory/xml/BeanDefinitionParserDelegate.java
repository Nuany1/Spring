...
public class BeanDefinitionParserDelegate {
	...
	public BeanDefinitionHolder parseBeanDefinitionElement(Element ele, @Nullable BeanDefinition containingBean) {
		...
		// 初始化GenericBeanDefinition 解析bean定义
	...
	public AbstractBeanDefinition parseBeanDefinitionElement(
		...
		try {
			// 初始化一个GenericBeanDefinition对象,下列解析的属性都会设置到bd中
			AbstractBeanDefinition bd = createBeanDefinition(className, parent);
			// 将给定 bean 元素的属性应用于给定 bean 定义
			parseBeanDefinitionAttributes(ele, beanName, containingBean, bd);
			// 解析给定元素下的 description元素,设置描述信息
			bd.setDescription(DomUtils.getChildElementValueByTagName(ele, DESCRIPTION_ELEMENT));
			// 解析给定元素下的 meta元素
			parseMetaElements(ele, bd);
			// 解析给定元素下的 lookup-method元素
			parseLookupOverrideSubElements(ele, bd.getMethodOverrides());
			// 解析给定元素下的 replaced-method元素
			parseReplacedMethodSubElements(ele, bd.getMethodOverrides());
			// 解析给定元素下的 constructor-arg元素
			parseConstructorArgElements(ele, bd);
			// 解析给定元素下的 property元素
			parsePropertyElements(ele, bd);
			// 解析给定元素下的 qualifier元素
	...
	public BeanDefinitionHolder decorateBeanDefinitionIfRequired(
		...
		// Decorate based on custom attributes first.根据自定义属性进行装饰
		NamedNodeMap attributes = ele.getAttributes();
		for (int i = 0; i < attributes.getLength(); i++) {
			Node node = attributes.item(i);
			finalDefinition = decorateIfRequired(node, finalDefinition, containingBd);
		}
		// Decorate based on custom nested elements.根据自定义子元素进行装饰
